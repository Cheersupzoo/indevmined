  ...
  return (
    <div
      className={cn(
        ...
        'rounded-2xl border border-amber-500/50 py-3 pl-6 pr-6 text-slate-700'
      )}
    >
      <div className='relative flex items-center'>
        ...
        <input
          ...
          className='mx-2 w-full bg-transparent text-slate-700 focus:outline-none'
        />
        <button
        ...
        <button
          className='cursor-pointer rounded-xl bg-amber-500/80 text-slate-100 disabled:bg-slate-600'
          disabled={isLoading}
          onClick={() => onSubmit()}
        >
          <CornerDownRight
      ...
      <div className='mx-16 h-[0.05rem] bg-amber-500/50' />
      ...
      <AutoAnimateHeight expanded={!isActive}>
        ...
        {exampleQuestions.map((question) => (
          <div
            ...
            className='mb-1 flex cursor-pointer items-start rounded-xl px-1 py-1 hover:bg-slate-200/15'
          >
            <div className='mr-2'>
      ...
      <div className='answer space-y-4 px-2'>
        {contents.map(({ value, type, loading, result }, index) => (
          <Fragment key={index}>
            {type === 'text' && <div>{value}</div>}
            {type === 'code' && (
              <EnterAnimateHeight className='text-sm'>
                <div className='rounded-t-[.3em] bg-slate-500 py-0.5 pl-2 text-xs text-slate-100'>
                  Python
                </div>
                <SyntaxHighlighter
                  PreTag='div'
                ...
                </SyntaxHighlighter>
                {
                  <div
                    className={cn(
                      'rounded-b-[.3em] bg-slate-300 py-0.5 pl-2 font-mono text-sm text-slate-700',
